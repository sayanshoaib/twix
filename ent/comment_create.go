...
func (cc *CommentCreate) SetTweetID(id int) *CommentCreate {
	cc.mutation.SetTweetID(id)
	return cc
}

// SetNillableTweetID sets the "tweet" edge to the Tweet entity by ID if the given value is not nil.
func (cc *CommentCreate) SetNillableTweetID(id *int) *CommentCreate {
	if id != nil {
		cc = cc.SetTweetID(*id)
	}
	return cc
...
func (cc *CommentCreate) SetTweet(t *Tweet) *CommentCreate {
	return cc.SetTweetID(t.ID)
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.TweetIDs(); len(nodes) > 0 {
