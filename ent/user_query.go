...
func (uq *UserQuery) QueryRates() *RateQuery {
...
func (uq *UserQuery) WithRates(opts ...func(*RateQuery)) *UserQuery {
	query := (&RateClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRates; query != nil {
		if err := uq.loadRates(ctx, query, nodes,
			func(n *User) { n.Edges.Rates = []*Rate{} },
			func(n *User, e *Rate) { n.Edges.Rates = append(n.Edges.Rates, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadRates(ctx context.Context, query *RateQuery, nodes []*User, init func(*User), assign func(*User, *Rate)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Rate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_rates
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_rates" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_rates" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
