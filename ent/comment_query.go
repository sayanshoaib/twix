...
func (cq *CommentQuery) QueryTweet() *TweetQuery {
	query := (&TweetClient{config: cq.config}).Query()
...
func (cq *CommentQuery) WithTweet(opts ...func(*TweetQuery)) *CommentQuery {
	query := (&TweetClient{config: cq.config}).Query()
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if cq.withTweet != nil || cq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withTweet; query != nil {
		if err := cq.loadTweet(ctx, query, nodes, nil,
			func(n *Comment, e *Tweet) { n.Edges.Tweet = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadTweet(ctx context.Context, query *TweetQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Tweet)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].tweet_comments == nil {
			continue
		}
		fk := *nodes[i].tweet_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tweet_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
