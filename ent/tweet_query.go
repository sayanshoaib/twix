...
func (tq *TweetQuery) QueryOwner() *UserQuery {
...
func (tq *TweetQuery) QueryRates() *RateQuery {
	query := (&RateClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TweetQuery) WithOwner(opts ...func(*UserQuery)) *TweetQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TweetQuery) WithRates(opts ...func(*RateQuery)) *TweetQuery {
	query := (&RateClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TweetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tweet, error) {
	...
	if query := tq.withOwner; query != nil {
		if err := tq.loadOwner(ctx, query, nodes, nil,
			func(n *Tweet, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withComments; query != nil {
		if err := tq.loadComments(ctx, query, nodes,
			func(n *Tweet) { n.Edges.Comments = []*Comment{} },
			func(n *Tweet, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withRates; query != nil {
		if err := tq.loadRates(ctx, query, nodes,
			func(n *Tweet) { n.Edges.Rates = []*Rate{} },
			func(n *Tweet, e *Rate) { n.Edges.Rates = append(n.Edges.Rates, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TweetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Tweet, init func(*Tweet), assign func(*Tweet, *User)) error {
...
func (tq *TweetQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Tweet, init func(*Tweet), assign func(*Tweet, *Comment)) error {
	...
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tweet.CommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tweet_comments
		if fk == nil {
			return fmt.Errorf(`foreign-key "tweet_comments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tweet_comments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TweetQuery) loadRates(ctx context.Context, query *RateQuery, nodes []*Tweet, init func(*Tweet), assign func(*Tweet, *Rate)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tweet)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Rate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tweet.RatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tweet_rates
		if fk == nil {
			return fmt.Errorf(`foreign-key "tweet_rates" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tweet_rates" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
