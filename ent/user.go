...
// was not loaded in eager-loading.
func (e UserEdges) RatesOrErr() ([]*Rate, error) {
	if e.loadedTypes[2] {
		return e.Rates, nil
	}
	...
	for i := range columns {
		...
		case user.FieldAge:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected types %T for field age", values[i])
			} else if value.Valid {
				u.Age = int(value.Int64)
			}
		case user.FieldName:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected types %T for field name", values[i])
			} else if value.Valid {
				u.Name = value.String
			}
		default:
...
// QueryRates queries the "rates" edge of the User entity.
func (u *User) QueryRates() *RateQuery {
	return NewUserClient(u.config).QueryRates(u)
}
