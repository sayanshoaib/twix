...
func (uc *UserCreate) AddRateIDs(ids ...int) *UserCreate {
	uc.mutation.AddRateIDs(ids...)
	return uc
}

// AddRates adds the "rates" edges to the Rate entity.
func (uc *UserCreate) AddRates(r ...*Rate) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return uc.AddRateIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.RatesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
